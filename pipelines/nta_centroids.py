...
def fetch_nta_centroids(conn):
    query = """
    WITH name_mapper AS (
        SELECT DISTINCT NTA_NAME, NTA_CODE
        FROM PERSONAL.PUBLIC.NTA_MAPPER
    ),
    centroids AS (
        SELECT NTA_CODE, ST_CENTROID(to_geography(geometry)) as geom, NTA_NAME
        FROM PERSONAL.PUBLIC.NTA_GEOGRAPHY
        LEFT JOIN name_mapper USING(NTA_CODE)
    )

    SELECT NTA_CODE, NTA_NAME, ST_Y(geom) AS LAT, ST_X(geom) AS LON
    FROM centroids;
    """
