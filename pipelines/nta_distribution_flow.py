...
def extract_transform(conn, feature_groups: dict[str, Any]) -> pd.DataFrame:
...
def load_to_snowflake(conn, data: pd.DataFrame, table_name: str):
...
def main():
    ...
    for feature, meta in COUNT_GROUPS.items():
        df = extract_transform(conn, meta)
        load_to_snowflake(conn, df, feature)
